...
export default function LiquidEther({
  ...
  useEffect(() => {
    ...
    const color_frag = `precision highp float;uniform sampler2D velocity;uniform sampler2D palette;uniform vec4 bgColor;varying vec2 uv;void main(){vec2 vel=texture2D(velocity,uv).xy;float rawLen=length(vel);float yBot=1.0-uv.y;float thick=1.0+yBot*1.4;float lenv=clamp(rawLen*thick,0.0,1.0);vec3 goldColor=texture2D(palette,vec2(lenv,0.5)).rgb;float blueStrength=uv.y*uv.y*0.45*clamp(rawLen*4.0,0.0,1.0);vec3 skyBlue=vec3(0.22,0.52,0.90);vec3 c=mix(goldColor,skyBlue,blueStrength);vec3 outRGB=mix(bgColor.rgb,c,lenv);float outA=mix(bgColor.a,1.0,lenv);gl_FragColor=vec4(outRGB,outA);}`
    ...
    class Simulation {
      ...
      createAllFBO() {
        ...
        for (let key in this.fbos) this.fbos[key] = new THREE.WebGLRenderTarget(this.fboSize.x, this.fboSize.y, opts as any)
      }
      createShaderPass() {
